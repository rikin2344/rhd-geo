#!/usr/bin/env python3
# update_6800_series_applications.py
import json
import os
from pathlib import Path

def update_6800_series_applications():
    """Update applications section in all 6800-6820 model JSON files"""
    
    # Load the applications data from the reference file
    applications_file = Path('docs/6200 Series Applications.json')
    models_dir = Path('models/')
    
    print('=== UPDATING 6800 SERIES APPLICATIONS ===\n')
    
    # Read the applications data
    with open(applications_file, 'r', encoding='utf-8') as f:
        applications_data = json.load(f)
    
    # Process each model from 6800 to 6820
    updated_count = 0
    total_count = 0
    
    for model_num in range(6800, 6821):
...
if __name__ == '__main__':
    update_6800_series_applications()
